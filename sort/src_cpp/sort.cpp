...
void merge_blocks(std::vector<uint32_t> &arr, std::vector<uint32_t> &scratch, int block_size)
{
  ...
  for (int i = blocks; i > 1; i >>= 1)
  {
...
#ifndef INPLACE
    std::vector<uint32_t> *main = (count % 2) ? &scratch : &arr;
    std::vector<uint32_t> *tmp = (count % 2) ? &arr : &scratch;
#endif
    ...
    for (int j = 0; j < (i >> 1); j++)
    {
...
#ifndef INPLACE
      std::vector<uint32_t>::iterator left_merge = main->begin() + (j << 1) * merge_block_size;
      ...
      std::merge(left_merge, mid_merge, mid_merge, right_merge, tmp->begin() + (j << 1) * merge_block_size);
#endif
